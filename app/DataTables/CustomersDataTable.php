    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($customer) {
                $name = app()->getLocale() === 'ar' ? $customer->name_ar : $customer->name_en; // Check the current language
                return view('components.datatable.actions', [
                    'routeEdit' => '',
                    'routeDelete' => 'customer.destroy',
                    'id' => $customer->id,
                    'name' => $name // Use the appropriate name based on language
                ]);
            })
            //service[]
            ->editColumn('service', function ($customer) {
                $services = explode(',', $customer->service);
                    $services = array_map(function ($service) {
                        return app()->getLocale() === 'ar' ?
                            Service::find($service)->name_ar ?? null : Service::find($service)->name_en ?? null; // Check the current language
                    }, $services);
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('phone')
                ...
                ->addClass('text-center align-middle'),
            Column::make('message_en')
                ->title(__('dataTable.message')) // Translate the title
                ->addClass('text-center align-middle'),
            //service[]
            Column::make('service')
                ->title(__('dataTable.service')) // Translate the title
                ->addClass('text-center align-middle'),
