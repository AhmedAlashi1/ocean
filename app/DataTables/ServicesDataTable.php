            ...
            ->addColumn('action', function ($service) {
                ...
                return view('components.datatable.actions',
                    [
                        ...
                        'ServiceDetails' => 'ServiceDetails.index',
                        'id' => $service->id,
                        'name' => $name
                    ]);
            })
            ->addColumn('image', function ($service) {
                return view('components.datatable.image', ['photo' => $service->image]);
            })

            ->addColumn('short_description', function ($row) {
                // إذا كان الحقل "desc" يحتوي على النص الكامل
                return Str::words($row->{'desc_' . (app()->getLocale() === 'ar' ? 'ar' : 'en')}, 5, '...');
            })
