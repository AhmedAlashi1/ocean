...
class TermsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($term) {
                $name = app()->getLocale() === 'ar' ? $term->name_ar : $term->name_en;
                return view('components.datatable.actions',
                    [
                        'routeEdit'=>'term.edit'
                        ,'routeDelete'=>'term.destroy'
                        ,'id' => $term->id,
                        'name' => $name
                    ]);
            })
            ->addColumn('created_at', function ($term) {
                return $term->created_at->format('Y-m-d H:i');
            })
            ->rawColumns(['action'])
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('service_name_' . (App::getLocale() === 'ar' ? 'ar' : 'en'))
