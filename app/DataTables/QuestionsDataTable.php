...
class QuestionsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($question) {
                $name = app()->getLocale() === 'ar' ? $question->question_ar : $question->question_en;
                return view('components.datatable.actions', [
                    'routeEdit' => 'question.edit',
                    'routeDelete' => 'question.destroy',
                    'id' => $question->id,
                    'name' => $name
                ]);
            })
            ->addColumn('created_at', function ($question) {
                return $question->created_at->format('Y-m-d H:i');
            })
            ->rawColumns(['action'])
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('question_' . (App::getLocale() === 'ar' ? 'ar' : 'en'))
                ->title(__('dataTable.question')) // Translate the title
                ->addClass('text-center align-middle'),
            Column::make('answer_' . (App::getLocale() === 'ar' ? 'ar' : 'en'))
                ->title(__('dataTable.answer')) // Translate the title
                ->addClass('text-center align-middle'),
