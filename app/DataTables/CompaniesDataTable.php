    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($company) {
                $name = app()->getLocale() === 'ar' ? $company->name_ar : $company->name_en; // Check the current language
                return view('components.datatable.actions', [
                    'routeEdit' => 'company.edit',
                    'routeDelete' => 'company.destroy',
                    'id' => $company->id,
                    'name' => $name // Use the appropriate name based on language
                ]);
            })
            ->addColumn('image', function ($company) {
                return view('components.datatable.image', ['photo' => $company->image]);
            })
            ->addColumn('created_at', function ($company) {
                return $company->created_at->format('Y-m-d H:i');
            })
            ->rawColumns(['action'])
